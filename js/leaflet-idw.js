    ...
    simpleidw.prototype = {
        ...
        _colorize: function (pixels, gradient, opacity) {
            for (var i = 0, len = pixels.length, j; i < len; i += 4) {
                ...
                //GoGs added to draw transparent pixels outside of mask
    ...
    L.IdwLayer = (L.Layer ? L.Layer : L.Class).extend({
        ...
        */
        ...
        onAdd: function (map) {
            ...
            //GoGs added for custom blur effect
            this._canvas.style.filter = 'blur(3px)'; // Adjust the blur radius as needed
        ...
        _redraw: function () {
            ...
            for (i = 0, len = nCellY; i < len; i++) {
                for (j = 0, len2 = nCellX; j < len2; j++) {
                    ...
                    // Check if the cell is within the GeoJSON mask
                    var cellLatLng = this._map.containerPointToLatLng(L.point(cell[0], cell[1]));
                    var cellPoint = turf.point([cellLatLng.lng, cellLatLng.lat]);
                    if (turf.booleanPointInPolygon(cellPoint, this.options.mask)) {
                        if (cell) {
                            data.push([
                                Math.round(cell[0]),
                                Math.round(cell[1]),
                        ...
                        // Add transparent cell
                        data.push([
                            Math.round(cell[0]),
                            Math.round(cell[1]),
                            0 // Set the value to 0 for transparency
                        ]);
                    }
